...
Many times your PHP code will use a database to persist information. If you use a database, use `PDO` to talk with it. PDO is a database access abstraction library &mdash; (usually) built into PHP &mdash; that provides a common interface to talk with many different databases.
...
Please be aware that PDO is not a layer that helps you dealing with all the different SQL dialects. The PDO layer is only a way to make accessing data in different database possible with the same API. In short, it's a unification and generalisation of all the different direct-access database extensions, such as oci8, mysql/mysqli and postgresql.
